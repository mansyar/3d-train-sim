...
- [ ] Task: Red — lock the chooser's rules with failing tests
  - [ ] Define the API in `src/core/park-spot.ts`: `findParkSpot(pieces)` →
        `{ kind: 'rails', pieceId, from, to } | { kind: 'land', cell }` — total,
        never null
  - [ ] `src/core/park-spot.test.ts`: cozy-oval → rail spot (dry, nearest heart
...
- [ ] Task: Green — implement `findParkSpot`
  - [ ] Largest ride via `selectRideComponents(rideComponentsOf(pieces), 1)`;
  ...
  - [ ] Land fallback: scan meadow cells in deterministic order → dry +
        nearest heart
  - [ ] Coverage check (`CI=true pnpm test -- --coverage`, >80% new module);
