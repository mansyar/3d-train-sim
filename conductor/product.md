...
- ~~Track switches and branches~~ — ✅ shipped (switches-branches_20260903, 2026-09-03): a Blender-authored Y-junction (`switch.glb`) with a straight through-road and a curved diverging branch; each train alternates branches every pass through the stem, point blades visibly flip to the chosen road, saves stay additive with no version bump. ✅ left-mirror shipped (mirror-switch_20260904, 2026-09-04): a second Blender-authored Y (`switch-mirror.glb`) branching west, with the same stem alternation and visible blade flips. ✅ three-way junction + motorized levers shipped (switchyard_20260913, 2026-09-14): a Blender-authored three-way piece (`switch-3way.glb`) where the stem cycles straight → right → left on successive passes, and every switch (both Ys and the three-way) grew a motorized signal lever that swings to point at the road the train will take as the points flip — no new controls, no reading, saves additive. Still roadmap: double-slip pieces.
...
- Music box — ✅ shipped (music-box_20260913, 2026-09-13): a singing town-tab toy, Blender-authored via a checked-in recipe with a winter snow cap — its figurine twirls and it chimes one of four public-domain nursery melodies (ABC/Twinkle, Mary Had a Little Lamb, London Bridge, Row Row Row Your Boat) as a riding train passes, never repeating the previous tune; the phrase finishes gently on ⏹, and every note is synthesized in the browser (zero new downloads).
