...
const mocks = vi.hoisted(() => {
	type Instance = Record<string, ReturnType<typeof vi.fn>>;
	const instances: Instance[] = [];
	// A constructible mock: Howler's Howl is created with `new`, so the fake
	// must be a real function (arrow functions cannot be constructed).
	const Howl = vi.fn(function (this: Instance, _options: unknown) {
		this.play = vi.fn(() => 1);
		this.stop = vi.fn();
		this.fade = vi.fn();
		this.once = vi.fn();
...
describe('createHowlerVoice', () => {
	...
	it('rate nudges playback tempo without restarting', () => {
		...
		expect(mocks.instances[0].rate).toHaveBeenCalledWith(0.85);
	});
