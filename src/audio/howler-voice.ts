...
const SOUNDS: Record<string, { base: string; loop: boolean; volume: number }> = {
  ...
  // Soft tick for rotation steps — under the other voices, never a sharp attack.
  click: { base: 'click', loop: false, volume: 0.7 },
  ...
  'ribbit-frog': { base: 'ribbit-frog', loop: false, volume: 0.5 },
...
export function createHowlerVoice(): AudioControllerOptions {
  ...
  function createSound(name: string): SoundHandle {
    ...
    return {
      ...
      volume: (value, id) => {
        if (id === undefined) howl.volume(value);
        else howl.volume(value, id);
      },
      onEnd: (listener) => {
